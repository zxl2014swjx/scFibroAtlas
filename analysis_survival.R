################################## START ##################################
# analysis_survival.R
# 输入: fibroblast_New_sample.csv, fibroblast_TCGA_samples.csv, TCGA_clinical_data.csv
...
# 加载新样本的成纤维细胞数据
new_samples_df<-read.table("1.Subtypes_deconvolution/deconvolution_scores.txt",header=T,sep="\t",row.names=1)
new_samples_df<-t(new_samples_df[,1:16])
# 加载TCGA样本的成纤维细胞数据
if(file.exists("Example_Data/fibroblast_TCGA_samples.csv")) {
  tcga_fibroblast <- read_csv("Example_Data/fibroblast_TCGA_samples.csv")
...
# 加载TCGA临床生存数据
if(file.exists("Example_Data/TCGA_clinical_data.csv")) {
  tcga_clinical <- read_csv("Example_Data/TCGA_clinical_data.csv")
